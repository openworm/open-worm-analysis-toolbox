    ...
    def normalize_all_frames_xy(heterocardinal_property, num_norm_points):
        ...
        for iFrame, cur_frame_value in enumerate(heterocardinal_property):
            if cur_frame_value is not None:
                ...
                cc = WormParserHelpers.chain_code_lengths_cum_sum(cur_frame_value2)

                # Normalize both the x and the y
                normalized_data[:,0,iFrame] = \
                    WormParserHelpers.normalize_parameter(cur_frame_value[0,:], cc, num_norm_points)
                normalized_data[:,1,iFrame] = \
                    WormParserHelpers.normalize_parameter(cur_frame_value[1,:], cc, num_norm_points)
    ...
    def normalize_all_frames(property_to_normalize, xy_data, num_norm_points):
        ...
        for frame_index, (cur_frame_value, cur_xy) in \
            ...
            if cur_xy is not None:
                ...
                running_lengths = \
                    WormParserHelpers.chain_code_lengths_cum_sum(cur_xy_reshaped)
